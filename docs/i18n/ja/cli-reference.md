...
新しいエージェントプロジェクトを、互いに排他的な3つの方法のいずれかで作成します: 組み込みテンプレートからスキャフォールドする、GitHubリポジトリをクローンする、または ForgeBox パッケージをインストールする。
...
# ForgeBox パッケージをインストールする
bxAgents new my-agent --forgebox=some-package-slug
```

- `--template`、`--repo`、`--forgebox` は3つの異なるプロジェクトの「ソース」を指定するもので、どれか1つだけが適用されます。2つを組み合わせるとエラーになります。
- **`--template` モード** (ソースオプションが何も渡されない場合のデフォルト - `minimal`):
...
- `--model`、`--repo`、`--forgebox` は、`--model` が必要なのは `--template` モードだけであるという意味で互いに排他的です。`--repo`/`--forgebox` では、ソースプロジェクトがすでに自身のものを定義しているため、決して必要とされません。
- `--repo`/`--forgebox` が使われる場合、`--name`/`--description` は (出力の警告付きで) 無視されます - クローン/インストールされたプロジェクトが自身のものを定義します。
- `tests/` の `box install` 利便性ステップ (および `--skipInstall`) は `--template` モードにのみ適用されます - クローン/インストールされたプロジェクト自身の `tests/` フォルダ (そもそも存在する場合) はそのまま残されます。
