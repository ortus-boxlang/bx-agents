...
- **`--template`-Modus** (der Standard, wenn überhaupt keine Quell-Option übergeben wird - `minimal`):
  ...
  - `--template` wählt eine von 8 eingebauten Vorlagen, die jeweils etwas vorkonfigurierten Inhalt auf das obige Basisgerüst legen:
    ...
    - `telegram-bot` - fügt einen `gateways/telegram.bx`-Kanaladapter hinzu, plus eine passende `TELEGRAM_BOT_TOKEN`-Platzhalterzeile in `.env`.
    - `github-bot` - fügt einen `gateways/github.bx`-Kanaladapter hinzu, plus passende `GITHUB_TOKEN`/`GITHUB_WEBHOOK_SECRET`/`GITHUB_BOT_NAME`-Platzhalterzeilen in `.env`.
    ...
    - `multi-agent` - fügt dem Root-`Agent.bx` ein `configure()` hinzu, das zwei [Subagenten](conventions/subagents.md) (`researcher`, `writer`) deklariert, jeder als eigenes echtes `subagents/<name>/Agent.bx` + `instructions.md` angelegt.
...
- Serviert den Agent immer unter `/__bxagents` (`POST /invoke`, `POST /stream`, `POST /batch`, `GET /info`), unabhängig davon, was das Projekt in [`gateways/`](conventions/gateways/index.md) deklariert - siehe [Die stets aktive Agent-Route](conventions/agent-bx.md#die-stets-aktive-agent-route), um ein Token dafür zu verlangen oder sie abzuschalten.
